...
TD(ESC_KEY),     KC_F1,    KC_F2,    KC_F3,    KC_F4,    KC_F5,    KC_F6,    KC_F7,    KC_F8,    KC_F9,    KC_F10,   KC_F11,   KC_F12,               TD(PRINT),  KC_MUTE,  RGB_MOD,
...
KC_LCTL,         KC_LOPTN, KC_LCMD,                                KC_SPC,                                 KC_RCMD,  KC_ROPTN, TD(FN_KEY), KC_RCTL,  KC_LEFT,    KC_DOWN,  KC_RGHT
),
...
void caps_finished(tap_dance_state_t *state, void *user_data) {
    ...
    switch (td_state) {
        case TD_SINGLE_TAP:
            ...
            // NOTE: Using add_oneshot_mods instead of set_oneshot_mods to ensure proper behavior
            add_oneshot_mods(MOD_BIT(KC_LCTL) | MOD_BIT(KC_LCMD) | MOD_BIT(KC_LOPT) | MOD_BIT(KC_LSFT));
            break;
        case TD_SINGLE_HOLD:
...
// ESC key tap dance implementation
void esc_finished(tap_dance_state_t *state, void *user_data) {
    td_state = cur_dance(state);
    switch (td_state) {
        case TD_SINGLE_TAP:
            ...
            break;
        case TD_DOUBLE_TAP:
            // On double tap: Send Ctrl+Cmd+Q
            register_mods(MOD_BIT(KC_LCTL) | MOD_BIT(KC_LCMD));
            register_code(KC_Q);
            break;
        default:
            break;
...
void esc_reset(tap_dance_state_t *state, void *user_data) {
    switch (td_state) {
        ...
        case TD_DOUBLE_TAP:
            ...
            unregister_mods(MOD_BIT(KC_LCTL) | MOD_BIT(KC_LCMD));
            break;
        default:
            break;
    }
...
// FN key tap dance implementation
...
tap_dance_action_t tap_dance_actions[] = {
    ...
    [ESC_KEY] = ACTION_TAP_DANCE_FN_ADVANCED(NULL, esc_finished, esc_reset)
};
